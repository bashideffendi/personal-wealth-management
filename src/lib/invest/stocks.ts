...
// Per-ticker lazy loader. The full universe (~30 MB) is split into
// src/data/invest/stocks/{TICKER}.json by scripts/split-stocks.mjs, so the
// server parses ONE ~20 KB file per request instead of holding the whole array
...
export function getStock(ticker: string): Stock | undefined {
  ...
  const cached = stockCache.get(t)
  if (cached !== undefined) return cached ?? undefined
  try {
    const raw = fs.readFileSync(path.join(STOCK_DIR, `${t}.json`), 'utf-8')
    const stock = JSON.parse(raw) as Stock
    stockCache.set(t, stock)
    return stock
  } catch {
    stockCache.set(t, null) // negative cache — don't re-stat a missing ticker
    return undefined
  }
}

/**
 * Precomputed sector medians (valuasi konsensus). Generated by
 * scripts/split-stocks.mjs from the full universe so detail pages don't have to
 * load + iterate all 1000+ stocks just to anchor one valuation. Output of
 * computeAllSectorMedians, frozen to a tiny (~1.6 KB) JSON.
 */
const SECTOR_MEDIANS = sectorMediansRaw as Record<string, SectorMedians>

export function getSectorMedians(): Record<string, SectorMedians> {
