...
/**
 * CreditCardsSection — kartu kredit = utang revolving, jadi dirender sebagai
 * SECTION di bawah halaman Utang (bukan halaman sendiri). Self-contained:
 * fetch data + dialog tambah kartu / bayar tagihan sendiri.
 *
 * Route lama /dashboard/credit-cards me-redirect ke /dashboard/debts#kartu-kredit.
 */
...
import { Plus, Pencil, Trash2, Loader2, Wallet, CreditCard } from 'lucide-react'
...
export function CreditCardsSection() {
  ...
  const utilColor = totals.utilization < 30 ? '#10B981' : totals.utilization < 70 ? '#F59E0B' : '#F43F5E'
  ...
  const stats = [
    { label: 'Total Tagihan', value: formatCurrency(totals.outstanding), color: totals.outstanding > 0 ? '#F43F5E' : 'var(--ink)' },
    { label: 'Total Limit', value: formatCurrency(totals.limit), color: 'var(--ink)' },
    { label: 'Sisa Limit', value: formatCurrency(totals.available), color: '#10B981' },
    { label: 'Utilisasi', value: `${totals.utilization.toFixed(0)}%`, color: utilColor },
  ]

  return (
    <section id="kartu-kredit" className="scroll-mt-24 space-y-4 pt-2 border-t" style={{ borderColor: 'var(--border-soft)' }}>
      {/* Section header */}
      <div className="flex flex-wrap items-end justify-between gap-3 pt-4">
        <div className="flex items-start gap-3 min-w-0">
          <div className="size-9 rounded-xl grid place-items-center shrink-0" style={{ background: 'rgba(244,63,94,0.12)' }}>
            <CreditCard className="size-5" style={{ color: '#F43F5E' }} />
          </div>
          <div className="min-w-0">
            <h2 className="text-xl leading-tight" style={{ fontFamily: 'var(--font-display)', color: 'var(--ink)' }}>Kartu Kredit</h2>
            <p className="text-sm mt-0.5" style={{ color: 'var(--ink-muted)' }}>
              Utang revolving — pantau tagihan, limit &amp; jatuh tempo. Jaga utilisasi di bawah 30%.
            </p>
          </div>
        </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12"><Loader2 className="h-5 w-5 animate-spin" style={{ color: 'var(--ink-soft)' }} /></div>
      ) : cards.length === 0 ? (
        <div className="s-card p-10 text-center">
          <CreditCard className="size-9 mx-auto" style={{ color: 'var(--ink-soft)' }} />
          <p className="font-semibold mt-3" style={{ color: 'var(--ink)' }}>Belum ada kartu kredit</p>
          <p className="text-sm mt-1" style={{ color: 'var(--ink-muted)' }}>Tambah kartu pertama buat lacak tagihan &amp; utilisasi.</p>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {stats.map((s) => (
              <div key={s.label} className="s-card p-4">
                ...
                <p className="num tabular text-xl font-bold mt-1 whitespace-nowrap" style={{ color: s.color }}>{s.value}</p>
              </div>
            ))}
          </div>
          ...
          <Tabs defaultValue="cards">
                  ...
                  {cards.map((c) => {
                    ...
                    return (
                      <div key={c.id} className="group flex flex-col gap-3">
                        ...
                        <div className="relative rounded-2xl p-5 overflow-hidden" style={{ background: gradient, color: '#FFFFFF', aspectRatio: '1.6 / 1', boxShadow: '0 8px 24px -8px rgba(15,23,42,0.35)' }}>
                          <div className="absolute top-4 right-4" style={{ width: 36, height: 28, borderRadius: 4, background: 'linear-gradient(135deg, rgba(255,255,255,0.85), rgba(255,255,255,0.50))', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.4)' }} />
                          <div className="absolute top-3 left-3 flex gap-1 opacity-0 group-hover:opacity-100 transition">
                            <button onClick={() => openEditCard(c)} aria-label="Edit" className="grid place-items-center rounded-md" style={{ width: 24, height: 24, background: 'rgba(255,255,255,0.18)', color: '#FFFFFF', backdropFilter: 'blur(4px)' }}>
                              <Pencil className="h-3 w-3" />
                            </button>
                            <button onClick={() => removeCard(c.id)} aria-label="Hapus" className="grid place-items-center rounded-md" style={{ width: 24, height: 24, background: 'rgba(255,255,255,0.18)', color: '#FFFFFF', backdropFilter: 'blur(4px)' }}>
                              <Trash2 className="h-3 w-3" />
                            </button>
                          </div>
                          <p className="absolute top-5 left-5" style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.85)' }}>{c.issuer}</p>
                          <div className="absolute bottom-12 left-5 right-5" style={{ fontFamily: 'var(--font-mono)', fontSize: 15, letterSpacing: '0.08em', color: 'rgba(255,255,255,0.95)' }}>
                            •••• •••• •••• {c.last_four || '••••'}
                          </div>
                          <div className="absolute bottom-4 left-5 right-5 flex items-end justify-between">
                            <div>
                              <p style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.65)' }}>Card holder</p>
                              <p style={{ fontSize: 13, fontWeight: 600, marginTop: 2, color: '#FFFFFF' }}>{c.name}</p>
                            </div>
                            <div className="text-right">
                              <p style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.65)' }}>Tagihan</p>
                              <p className="num tabular" style={{ fontSize: 14, fontWeight: 700, marginTop: 2, color: '#FFFFFF' }}>{formatCurrency(c.current_balance)}</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="s-card px-4 py-3.5">
                          <div className="flex items-center justify-between">
                            ...
                            <p className="num tabular" style={{ fontSize: 11, color: 'var(--ink-muted)' }}>{util.toFixed(0)}% · {c.interest_rate}% bunga</p>
                          </div>
                          <div className="kl-bar mt-2" style={{ color: util > 80 ? 'var(--c-coral)' : util > 50 ? 'var(--c-amber)' : 'var(--c-mint)' }}>
                          ...
                          <div className="mt-3 pt-3 border-t flex items-center justify-between" style={{ borderColor: 'var(--line)' }}>
                            <div>
                              ...
                              <p className="num tabular font-medium mt-0.5" style={{ fontSize: 12, color: urgency }}>
                                {formatDate(due.toISOString())}<span className="ml-1 opacity-70">({daysLeft}h lagi)</span>
                              </p>
                            </div>
                            <Button size="sm" variant="outline" onClick={() => openPayCard(c)} disabled={c.current_balance === 0} className="btn-outline">Bayar</Button>
                          </div>
                        </div>
                      </div>
                    )
            ...
            <TabsContent value="payments">
              <div className="pt-4">
                {payments.length === 0 ? (
                  <div className="s-card p-10 text-center">
                    <p className="font-semibold" style={{ color: 'var(--ink)' }}>Belum ada riwayat pembayaran</p>
                    <p className="text-sm mt-1" style={{ color: 'var(--ink-muted)' }}>Pembayaran tagihan bakal kecatat di sini.</p>
                  </div>
                ) : (
                      ...
                      {payments.map((p) => {
                        ...
                        return (
                          <div key={p.id} className="flex items-center gap-4 px-5 py-4 hover:bg-[var(--surface-2)]/60 transition-colors">
                            ...
                            <p className="num font-semibold tabular" style={{ color: 'var(--ink)' }}>{formatCurrency(p.amount)}</p>
                          </div>
                        )
                      })}
                    </div>
